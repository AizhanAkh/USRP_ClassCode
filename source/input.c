...
int input_read_parameters(
  ...
  */
  ...
  class_call(parser_read_string(pfc,"bcdm coupling",&(string1),&(flag1),errmsg),errmsg, errmsg);
  if(flag1 == _TRUE_){
    if( (strstr(string1,"y")!=NULL) || (strstr(string1,"Y") !=NULL)) {
      pth->has_coupling_bcdm=_TRUE_;
    }
    else{
      if((strstr(string1,"n")!=NULL) || (strstr(string1,"N") !=NULL)) {
	...
	class_stop(errmsg,"incomprehensible input '%s' for the field 'bcdm coupling' ", string1);
      }
    }
  }
