...
int input_read_parameters(
  ...
  class_call(parser_read_double(pfc,"omega_b",&param2,&flag2,errmsg),
             errmsg,
             errmsg);
  /* Read the baryon fraction eta_b from exercise 1e: */
  ...
  class_test(class_at_least_two_of_three(flag1,flag2,flag3),
             errmsg,
             "In input file, you can only enter one of Omega_b or omega_b or eta_b, choose one");
  if (flag1 == _TRUE_)
    pba->Omega0_b = param1;
  if (flag2 == _TRUE_)
  ...
  class_read_double("m_deff",pba->m_dmeff);
  class_read_double("n_power_dmeff",pba->npow_dmeff);
  class_read_double("F_he_dmeff",pba->F_he_dmeff);
  class_read_double("sigma_0_dmeff",pba->sigma_0_dmeff);
  class_read_double("f_he_dmeff", pba->f_he_dmeff);
  ...
  */
  ...
  class_call(parser_read_string(pfc,"has_dmeff",&(string1),&(flag1),errmsg),errmsg, errmsg);
  if(flag1 == _TRUE_){
    if( (strstr(string1,"y")!=NULL) || (strstr(string1,"Y") !=NULL)) {
	...
	class_stop(errmsg,"print 'Y' or 'N', not '%s' ", string1);
      }
    }
  }
